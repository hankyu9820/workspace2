...
document.getElementById("select1").addEventListener("click",function(){
    ...
    $.ajax({
        ...
        success : function(member){
            ...
            // 문자열을 JSON.parse(문자열) : 문자열 -> JS 객체로 변환
           ...
           if(member != null){ // 회원 정보 존재 O
                ...
                // 3) li 요소 생성 * 5 + 내용 추가
                const li1 = document.createElement("li"); // li라는 요소를 만들겠다.
                ...
                ul.append(li1, li2, li3, li4, li5); // 연달아서도 쓸 수 있다.
...
/* function selectAll(){ // 회원 전체 조회 함수
    ...
    $.ajax({
        ...
        dataType : "json", // 응답 데이터의 형식을 "json으로 지정" -> 자동으로 JS 객체로 변환되어 받아진다.
        ...
        success : function(memberList){
           ...
           for(let item of memberList){
                ...
                const td1 = document.createElemend("td1"); 
                td1.innerText = item.memberNo; // 회원 번호

                const td2 = document.createElemend("td2");
                td2.innerText = item.memberEmail; // 회원 이메일

                const td3 = document.createElemend("td3");
...
(function(){
    ...
    window.setInterval(selectAll, 10000); // 10초
                     // 함수이름만 작성하면 함수 안의 코드가 대입.
                     // 10초마다 selectAll 함수 수행

    // setInterval()은 지연 -> 수행 -> 지연 -> 수행 의 반복
    // -> 처음에 함수가 수행되지 않아서 공백인 상태가 있다.
