...
public class myPageInfoServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Member loginMember = (Member)session.getAttribute("loginMember"); // 여기서 loginmember꺼 속성 다 얻어온듯
		/*
		 * System.out.println(loginMember);
		 */
		int memberNo = loginMember.getMemberNo(); // 회원 정보 얻어오기 성공 다른건 parameter로 얻어온듯
		...
		try {
			...
			int result = service.updateMember(mem); // update니까 int result로 반환받음
													// 배열에 담던지 뭘 하는건 이미 담겨져잇음 로그인 정보가 수정만 하는거
			if(result > 0 ) { //성공
				System.out.println("성공");
				// footer.jsp에 alert(${message})가 있다
			...
			// 절대경로
			resp.sendRedirect(req.getContextPath() + "/member/myPage/info"); // 최상위 주소
