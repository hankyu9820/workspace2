...
public class Pagination {
	...
	private int prevPage;		// 목록 하단에 노출된 번호의 이적 목록 끝 번호
	...
	private void calculatePagination() {
		...
		// 전체 게시글의 수 : 500개 // 보여지는 게시글의 수 : 10개
		// -> 마지막 페이지 번호는? 500 / 10 = 50
		
		// 전체 게시글의 수 : 501개 // 보여지는 게시글의 수 : 10개
		// -> 마지막 페이지 번호는? 501 / 10 = 50.1 -> 51 올림처리 해줘야 한다.
		
		maxPage = (int)Math.ceil(((double)listCount / limit)); // limit = 10
		...
		// (currentPage - 1) / pageSize 에서, 1~10 = 0나옴 // 1~11 = 1나옴 (int는 소수점 버리니까) - 1 은 10, 20, ... 때문에 한듯
		// * pageSize + 1 ; 1~10 = 0 x 10 + 1 = 1 // 11~20 1 x 10 + 1 = 11 // 21~30 2 x 10 + 1 = 21 
		...
		if(endPage > maxPage) { // n x 10 번째 페이지가 없는 경우 조건을 걸어줘야함
			endPage = maxPage;
		}
		
		// * prevPage(<) : 목록 하단에 노출된 번호의 이전에 목록 끝 번호
		...
		// 현재 페이지가 11~20일 경우 
		// < : 10 페이지 <-이전 목록(1~10)의 끝번호
		// > : 21 페이지
		
		// 현재 페이지가 41~50일 경우 (maxPage가 50이라고 가정)
		// < : 40 페이지
		// > : 50 페이지
		
		if(currentPage <= pageSize) { // 현재페이지가 페이지 사이즈보다 작으면 (1~10 페이지)
			prevPage = 1;
		}else {
