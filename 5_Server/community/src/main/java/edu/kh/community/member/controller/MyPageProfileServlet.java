...
public class MyPageProfileServlet extends HttpServlet {
   ...
   protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
      ...
      try {
         ...
         // 파라미터가 얻어와지지 않는 이유
         // 1. enctype = "multipart/form-data" -> 인코딩이 안되어 있어 파라미터가 인지되지 않음
         ...
         int maxSize = 1024 * 1024 * 20; // 최대크기는 20메가바이트
         			// 1KB     1MB   20MB
         ...
         String root = session.getServletContext().getRealPath("/"); // /community/src/main/webapp?
         ...
         // -> cos.jar에서 제공하는 FileRenamePolicy 인터페이스 상속 클래스 생성 (MyRenamePolicy)
         ...
         // 그냥 변경하기 누르면 null 값이 뜨면서 사라짐 js 코딩필요
         ...
         //     req를 이용해서 파라미터를 얻어올 수 없음.
         //		-> mpReq를 이용하면 가능하다.
         
         int delete = Integer.parseInt(mpReq.getParameter("delete"));
         
         // delete의 값이 1(눌러진 경우) 이면 profileImage의 값을 null로 변경.
         if (delete == 1) {
        	 profileImage = null;
         }
         ...
         if(result > 0 ) { // 성공
            ...
            // Session에 저장된 로그인정보 중 프로필 이미지는 변경되지 않음
            // -> 동기화 작업 진행 (내 정보 수정 참고)
            
            loginMember.setProfileImage(profileImage); // 세션의 값이 변경됨
            
         } else {
            session.setAttribute("message",   "프로필 이미지 변경 실패ㅜㅜㅜㅜㅜㅜㅜㅜ");
         }
