...
public class EncryptWrapper extends HttpServletRequestWrapper {
   ...
   // 해시 함수 : 어떤 문자열이든 일정한 길이의 무작위 문자열로 변환하는 함수(중복x)
	 ...
	 * method stub return super.getParameter(name); }
	 */

   
	// getParameter() 오버라이딩
   // 암튼 오버라이딩하면 전체 name들을 가져오는데 거기서 pw 2개만 암호화 하는 느낌??
	@Override
	public String getParameter(String name) {
		...
		switch(name) {
		/* getParameter("inputPw") 원래 작성법*/
		case "inputPw" : 
		case "memberPw" : 
		case "currentPw" : 
		case "newPw" : 
			value = getSha512(super.getParameter(name)); // Pw일때만
			break;
