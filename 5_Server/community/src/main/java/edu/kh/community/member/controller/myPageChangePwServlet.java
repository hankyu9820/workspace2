...
public class myPageChangePwServlet extends HttpServlet{
	...
	// get방식 요청 : /WEB-INF/views/member/myPage-changePw.jsp로 요청 위임
	@Override
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 1. 매개변수로 하나하나 따로 전달한다. 밑에처럼+
		// 		-> 최대 4개를 넘지 않는 것을 권장한다.
		
		// 2. VO 새로 만들기(1회성으로 사용하면 비효율적이다)
		...
		try {
			...
			if(result > 0) {
				...
				session.setAttribute("message", "비밀번호 실패!");
				// path = "비밀번호 변경 페이지 주소"
