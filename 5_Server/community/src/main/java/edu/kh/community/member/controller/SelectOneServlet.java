...
public class SelectOneServlet extends HttpServlet {
   ...
   protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
      ...
      try {
         ...
         // 해당 참조형 변수의 toString() 메소드를 자동 호출해서 출력한다.
         
         // *** Java 객체를 Javascript 객체로 변환하여 응답(출력) ***
         
         // Java 객체 -> Javascript 객체 형태의 문자열(JSON) -> Javascript 객체로 전달해줘야 한다.
         
         // 1) JSON 직접 작성 -> 오타 너무 많이 남
         // String str = "{\"memberNickname\" : \""+ member.getMemberNickname() +  "\"}";
         //resp.getWriter().print(str);
         
         // 2) JSON-Simple 라이브러리에서 제공하는 JSONObject를 사용하겠다.
			 ...
			 * 
			 * // JSONObject의 toString() 메소드는 // JSON 형태로 출력될 수 있도록 오버라이딩이 되어있다.
			 * resp.getWriter().print(obj.toString()); // obj 해도 ㄱㄴ
			 * 
			 * }else {
			 * 
       ...
       //resp.getWriter().print(str); <- 원래 하던거
